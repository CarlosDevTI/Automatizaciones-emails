...
from reports.data_processor import build_branch_performance, calculate_compliance_pct, normalize_records
...
class DataProcessorTests(SimpleTestCase):
    def test_normalize_records_aggregates_duplicate_branches(self):
        records = normalize_records(
            [
                {"branch_code": 101, "branch_name": "Principal", "current_amount": Decimal("100"), "monthly_target": Decimal("80")},
                {"branch_code": 101, "branch_name": "Principal", "current_amount": Decimal("50"), "monthly_target": Decimal("20")},
                {"branch_code": 102, "branch_name": "Popular", "current_amount": Decimal("75"), "monthly_target": Decimal("100")},
            ]
        )
    ...
    def test_build_branch_performance_calculates_summary_rank_and_compliance(self):
        normalized = normalize_records(
            [
                {"branch_code": 101, "branch_name": "Principal", "current_amount": Decimal("300"), "monthly_target": Decimal("200")},
                {"branch_code": 102, "branch_name": "Popular", "current_amount": Decimal("100"), "monthly_target": Decimal("150")},
            ]
        )
